...
- (BOOL)webView:(UIWebView*)theWebView shouldStartLoadWithRequest:(NSURLRequest*)request navigationType:(UIWebViewNavigationType)navigationType
{
    ...
    for (NSString* pluginName in pluginObjects) {
        ...
        if ([plugin respondsToSelector:selector]) {
            ...
            if (((BOOL (*)(id, SEL, id, int))objc_msgSend)(plugin, selector, request, navigationType) == YES) { //http://shazronatadobe.wordpress.com/2014/03/12/xcode-5-1-and-cordova-ios/#comment-5877
                return NO;
            }
        }
    }
...
- (void)webToNativeCall
{
    ...
    NSString *rebDeviceToken = [[NSUserDefaults standardUserDefaults] stringForKey:@"REBDeviceToken"];
    
    NSString *uuid=[[[UIDevice currentDevice] identifierForVendor] UUIDString];
    NSLog(@"UUID %@",uuid);
    
    NSString *returnvalue=[self.webView stringByEvaluatingJavaScriptFromString:[NSString stringWithFormat:@"getPushNotificationData('%@','%d','%@')", rebDeviceToken,regdForPush,uuid]];
    NSLog(@"RET Value: %@",returnvalue);
